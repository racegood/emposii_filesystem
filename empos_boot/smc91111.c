/*------------------------------------------------------------------------
 ...
 .	03/16/01  Daris A Nevil    Modified smc9194.c for use with LAN91C111
 	2004/7/10  Yang, Keun-Bae    Modified smc91111.c for use with EMPOS board
 ----------------------------------------------------------------------------*/
...
#define M1		1*100//  1.8/sec
#define M10		M1*6	// 10u/sec
#define M100		M10*10	// 100u/sec
#define M1000		M10*100	// 1000u/sec
...
static char smc_mac_addr[] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
...
static int poll4int( byte mask, int timeout ) {
    time_t now;
    int tmo = GetTime(&now) + timeout * CFG_HZ;
...
int smc_send_packet(void *packet, int packet_length)
{
	byte	 		packet_no;
...
static int smc_open()
{
	...
	long int base_address_register;
	PRINTK2("%s:smc_open\n", SMC_DEV_NAME);
	...
	if((SMC_BASE_ADDRESS !=0x4000300) && (SMC_BASE_ADDRESS !=0x8000300)) {
  		printf("LAN91C111 base address is not correct\n");
		return 0;
	}


	if ((SMC_BASE_ADDRESS & 0x00000fff) != ( base_address_register >> 3 & 0x3E0 ) )
	{
	  printf("LAN91C111 is not probed\n");
	  return 0;
	}else{
              if(SMC_BASE_ADDRESS ==0x4000300){
	          printf("[eth0] Primary LAN91C111 is OK.!\n");
		  macoff =0x0;
	      }
	      else if(SMC_BASE_ADDRESS == 0x8000300) {
	          printf("[eth1] Secondary LAN91C111 is OK.!\n");
		   macoff =0x6;
	      }
	}
	...
	for(i=0;i<6;i++)
		clientEther[i] =  __REG8(CONFIG_ROM_BASE + macoff +i);
...
//	SMC_outw(0, RPC_REG);
	...
	for ( i = 0; i < 6; i += 2 ) {
		...
		address = clientEther[ i + 1 ] << 8 ;
		address  |= clientEther[ i ];
		SMC_outw( address, ADDR0_REG + i );
	}
#else

	for ( i = 0; i < 6; i ++ )
		SMC_outb( clientEther[i], ADDR0_REG + i );
#endif
...
int smc_rcv(volatile void *packet)
{
	int 	packet_number;
	word	status;
	word	packet_length;
	...
	if ( !(status & RS_ERRORS ) ){
		...
		SMC_insl(SMC91111_DATA_REG , &data, packet_length >> 2);

		SMC_insb(SMC91111_DATA_REG, &data + (packet_length & 0xFFFFFC), (packet_length & 0x3)  );

#else
		PRINTK3(" Reading %d words and %d byte(s) \n",
			(packet_length >> 1 ), packet_length & 1 );

		SMC_insw(SMC91111_DATA_REG , &data, packet_length >> 1);
	...
	if (!is_error) {
		/* Pass the packet up to the protocol layers. */
 		...
 		RxPacketHandle(data, packet_length);
...
static void smc_phy_configure()
{
	...
	SMC_outw(RPC_SPEED | RPC_DPLX | RPC_ANEG | (RPC_LED_TX_RX <<RPC_LSXB_SHFT), RPC_REG );
...
int eth_send(volatile void *packet, int length) {
	smc_send_packet((void *)packet, length);
	return 1;
}
